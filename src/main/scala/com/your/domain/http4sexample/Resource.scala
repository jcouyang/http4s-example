...
object AppResource {
  def apply(implicit ctx: ContextShift[IO]): Resource[IO, AppResource] = for {
  ...
  private def mkHttp4sClient(uri: Uri)(implicit ctx: ContextShift[IO]): Resource[IO, Client[IO]] = {
    (uri.scheme, uri.host, uri.port) match {
      case (Some(Scheme.https), Some(host), None) => 
        Finagle.mkClient[IO](Http
          .client
        .withTls(host.value)
        .withHttpStats
          .newService(s"$host:443"))
      case (Some(Scheme.https), Some(host), Some(port)) =>Finagle.mkClient[IO](
        Http
          .client
        .withTls(host.value)
        .withHttpStats
          .newService(s"$host:$port"))
      case (_, Some(host), Some(port)) =>Finagle.mkClient[IO](
        Http
          .client
          .withHttpStats
          .newService(s"$host:$port"))
      case _ => Resource.liftF(IO.raiseError(new Exception(s"cannot initialize HttpClient for $uri")))
    }
  }
}
